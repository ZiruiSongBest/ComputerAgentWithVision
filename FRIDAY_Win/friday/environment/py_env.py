...
class PythonEnv(Env):
    ...
    def step(self, _command: str, args: list[str] | str = []) -> EnvState:
        ...
        # params_filename = f"params_{id(self)}_{int(time.time())}.txt"
        # params_file_path = os.path.join(self.working_dir, params_filename)

        # with open(params_file_path, "w", encoding="utf-8") as f:
        #     f.write(f"_command: {_command}\n")
        #     f.write(f"args: {args}\n")
...
if __name__ == '__main__':
    ...
    # print(env.observe('C:\Users\dylan\Desktop\ComputerAgentWithVision\FRIDAY_Win'))
    # print(env.step("cd ../../"))
    # print(env.step("gogo"))
    # env.reset()
    # print(env.step("timeout /t 3"))